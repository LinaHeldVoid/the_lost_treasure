...
def room_1(t_settings, v_a_settings, v_p_settings, m_settings, s_settings):
    ...
    g = generate_base('../text/2_room_1.txt')
    print(next(g))
    s_e('../sound/voice_actions/4_room_1.wav', v_a_settings)
    time.sleep(2)
    ...
    while True:
        ...
        record = s_e('../sound/voice_actions/5_look_around.wav', v_a_settings)
    ...
    while True:
        if chest_is_opened:
            ...
            s_e('../sound/voice_actions/option.wav', v_a_settings)
            time.sleep(2) if not t_settings and v_a_settings else None
            record = s_e('../sound/voice_actions/12_menu_2.wav', v_a_settings)
            option = input('Введите цифру: ')
            ...
            s_e('../sound/voice_actions/option.wav', v_a_settings)
            time.sleep(2) if not t_settings and v_a_settings else None
            record = s_e('../sound/voice_actions/6_menu_1.wav', v_a_settings)
            option = input('Введите цифру: ')
            record.stop() if v_a_settings else None
        if option == '100':
            q(t_settings, v_a_settings)
            ...
            while True:
                ...
                s_e('../sound/voice_actions/option.wav', v_a_settings)
                time.sleep(2) if not t_settings and v_a_settings else None
                record = s_e('../sound/voice_actions/7_bowl_options.wav', v_a_settings)
                option = input('Введите цифру: ')
                record.stop() if v_a_settings else None
                ...
                if option == '1':
                    while True:
                        ...
                        s_e('../sound/s_es/death.wav', s_settings)
                        ...
                        print(f'1) {next(g)}'
                              f'2) {next(g)}' + '\n')
                        record = s_e('../sound/voice_actions/10_death_menu.wav', v_a_settings)
                ...
                while True:
                    ...
                    if diamond_is_taken:
                        ...
                        print(f'1) {next(g)}'
                              ...
                              f'4) {next(g)}'
                              f'5) {next(g)}' + '\n')
                        record = s_e('../sound/voice_actions/17_chest_inside.wav', v_a_settings)
                    else:
                        g = set_generator(59)
                        print(f'1) {next(g)}'
                              f'2) {next(g)}'                  
                              ...
                              f'5) {next(g)}'
                              f'6) {next(g)}' + '\n')
                        record = s_e('../sound/voice_actions/17_chest_opened_no_diamond.wav', v_a_settings)
                    option = input('Введите цифру: ')
                    record.stop() if v_a_settings else None
                    ...
                    if option == '1':
                        ...
                        while True:
                            if bowl_is_examined:
                                if twig_melted:
                                    ...
                                    s_e('../sound/voice_actions/option.wav', v_a_settings)
                                    time.sleep(2) if not t_settings and v_a_settings else None
                                    record = s_e('../sound/voice_actions/9_twig_options.wav', v_a_settings)
                                    option = input('Введите цифру: ')
                                    record.stop() if v_a_settings else None
                                    if option == '1':
                                        g = set_generator(80)
                        ...
                        if stonewall_is_examined:
                            ...
                            while True:
                                ...
                                s_e('../sound/voice_actions/option.wav', v_a_settings)
                                time.sleep(2) if not t_settings and v_a_settings else None
                                record = s_e('../sound/voice_actions/14_smash_menu.wav', v_a_settings)
                                option = input('Введите цифру: ')
                                record.stop() if v_a_settings else None
                                if option == '1':
                                    g = set_generator(106)
                                    ...
                                    while True:
                                        ...
                                        s_e('../sound/voice_actions/option.wav', v_a_settings)
                                        time.sleep(2) if not t_settings and v_a_settings else None
                                        record = s_e('../sound/voice_actions/16_final_menu.wav', v_a_settings)
                                        option = input('Введите цифру: ')
                                        record.stop() if v_a_settings else None
                                        if option == '1':
                                            g = set_generator(125)
                                            ...
                                            while True:
                                                if diamond_is_taken:
                                                    ...
                                                    s_e('../sound/voice_actions/option.wav', v_a_settings)
                                                    time.sleep(2) if not t_settings and v_a_settings else None
                                                    record = s_e('../sound/voice_actions/18_fm_2.wav',
                                                                          v_a_settings)
                                                    option = input('Введите цифру: ')
                                                    record.stop() if v_a_settings else None
                                                    if option == '1':
                                                    ...
                                                    s_e('../sound/voice_actions/option.wav', v_a_settings)
                                                    time.sleep(2) if not t_settings and v_a_settings else None
                                                    record = s_e('../sound/voice_actions/19_fm_3.wav',
                                                                          v_a_settings)
                                                    option = input('Введите цифру: ')
                                                    record.stop() if v_a_settings else None
                                                    if option == '1':
                                            ...
                                            if diamond_is_taken:
                                                ...
                                                g = set_generator(135)
                                                s_e('../sound/voice_person/room_1/35_need_diamond.wav',
                                                             v_p_settings)
                                                p_e(next(g), t_settings)
            ...
            if stone_moved:
                ...
                while True:
                    ...
                    s_e('../sound/voice_actions/option.wav', v_a_settings)
                    time.sleep(2) if not t_settings and v_a_settings else None
                    record = s_e('../sound/voice_actions/13_stone_menu.wav', v_a_settings)
                    option = input('Введите цифру: ')
                    record.stop() if v_a_settings else None
                    if option == '1':
                        g = set_generator(54)
                        ...
                        while True:
                            ...
                            s_e('../sound/voice_actions/option.wav', v_a_settings)
                            time.sleep(2) if not t_settings and v_a_settings else None
                            record = s_e('../sound/voice_actions/17_open_chest_menu.wav', v_a_settings)
                            option = input('Введите цифру: ')
                            record.stop() if v_a_settings else None
                            if option == '1':
                                g = set_generator(57)
...
room_1(t_settings=False, v_a_settings=False, v_p_settings=False, m_settings=False, s_settings=False)
